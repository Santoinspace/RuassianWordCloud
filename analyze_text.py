...
def process_russian_text(texts: List[str]) -> List[Tuple[str, str]]:
    ...
    for text in texts:
        # 预替换：将多词专有名词合并为连写占位词，防止分词时被拆散
        text = re.sub(r'чжан\s+сюэ-лян|чжан\s+сюэлян|чжансюэ-лян', 'чжансюэлян', text, flags=re.IGNORECASE)
        text = re.sub(r'чан\s+кай-ши|чан\s+кайши|чанкай-ши', 'чанкайши', text, flags=re.IGNORECASE)
...
def generate_wordcloud(words: List[Tuple[str, str]], title: str, output_file: str, output_dir: str = None, font_path: str = None):
    ...
    参数:
        words: [(词汇, 词性), ...] 列表，或 {词汇: 频次} 字典（用于镜像词云）
        title: 图表标题
        output_file: 输出文件名
    ...
    # 支持直接传入词频字典（镜像词云）或元组列表
    if isinstance(words, dict):
        word_freq = words
    else:
        word_freq = Counter(w[0] for w in words)
